...
test.describe('엑셀 빌더 골든 (SLS-1-135)', () => {
    ...
    test('흙토람 buildWorksheetData — 41열 컬럼 매핑 골든', async ({ page }) => {
        const res = await buildAoa(page, 'buildWorksheetData');
        expect(res.error, res.error).toBeUndefined();
        const aoa = res.aoa;
        // 구조 불변식: 헤더 4행 + 데이터 3행 = 7행, 데이터행 41열(헤더 행도 41열)
        expect(aoa).toHaveLength(7);
        for (let r = 0; r < 4; r++) expect(aoa[r]).toHaveLength(41);
        for (let r = 4; r < 7; r++) expect(aoa[r]).toHaveLength(41);
        // 헤더 제목 행 고정
...
const GOLDEN_HEUKTORAM = [
    ["필지","2026","2026-05-01","농가의뢰","논","일반적인토양검정-0","","503","경상북도","봉화군","봉화읍","삼계리","","123","","","","1000","2026-05-10","벼","미해당","12","6.5","25","350","0.8","6.0","1.5","120","1.2","300","5","10","2","","","","","","",""],
    ["하위필지","2026","2026-05-01","공익직불제","논","일반적인토양검정-0","","503","경상북도","봉화군","봉화읍","문단리","산","45","2","","인근",992,"2026-05-11","고추","미해당","","7.1","30","","","","","","","","","","","","","","","","",""],
    ["필지","2026","2026-05-02","농가의뢰","밭","일반적인토양검정-0","","F503","경상북도","봉화군","물야면","오록리","","78","","","","","2026-05-12","","해당","","6.0","","","","","","","","","","","","","","","","","",""],
];
