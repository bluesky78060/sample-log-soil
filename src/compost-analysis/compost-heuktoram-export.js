...
(function () {
    ...
    /**
     * 부숙도 정규화. `완전부숙`은 어느 법령에도 없고 양식 5종에도 없어
     * SLS-1-207에서 선택지에서 뺐다 — 남은 항목은 오래된 백업 복원용 관용이다.
     *
     * ⚠️ MATURITY_ORDER 값을 코드(01~05) 파생에 쓰지 말 것. 지금은 순번이 우연히
     *    01~05와 맞지만, 등급이 하나 추가되거나 순서가 바뀌면 코드가 조용히 어긋난다.
     *    양식이 요구하는 코드는 등급 순서와 별개 계약이므로 매핑을 명시적으로 둔다.
     */
    const MATURITY_MAP = {
