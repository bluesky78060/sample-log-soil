...
describe('행별 1차 읽기', () => {
    ...
    it('7. 목록에 없는 값은 오류 행이 되고, 사유에 그 값과 빠져나갈 길이 있다', () => {
        const p = preview([['홍길동', '직불(특수)', '논']])
        expect(p.stats.err).toBe(1)
        expect(p.willImport).toBe(0)
        expect(p.items[0].reason).toContain('직불(특수)')
        expect(p.items[0].reason, '고칠 방법을 안 알려 준다').toMatch(/매핑을 해제|엑셀을 고치/)
    })
})

// ══════════════════════════════════════════════════════════════
// SLS-1-239 — 흙토람 표기 '직불(일반)'을 표준값으로 되돌린다
//
// 🚨 우리가 나눠 준 서식의 공익직불제 시트에 1차가 '직불(일반)'로 적혀 있다.
//    내보낼 때는 '공익직불제' → '직불(일반)' 변환이 있는데(heuktoram-script.js)
//    들어올 때가 없어 **전 행이 오류**가 났다. 왕복의 빠진 짝을 채운 것이다.
// ══════════════════════════════════════════════════════════════
describe("흙토람 표기 '직불(일반)' 별칭", () => {
    it('15. 오류가 아니라 표준값 공익직불제로 저장된다', () => {
        const p = preview([['홍길동', '직불(일반)', '논']])
        expect(p.stats.err, '서식의 예시값이 아직 오류로 처리된다').toBe(0)
        expect(p.items[0].rec.landClass1, '별칭을 표준값으로 안 바꿨다').toBe('공익직불제')
        expect(p.willImport).toBe(1)
    })

    it('16. 공백 변형도 받아들인다', () => {
        for (const v of ['직불 (일반)', ' 직불(일반) ', '직불(일반)\n']) {
            const p = preview([['홍길동', v, '논']])
            expect(p.items[0].rec.landClass1, `'${v}'를 못 읽는다`).toBe('공익직불제')
        }
    })

    // 🚨 목록에 '직불'이 **별개 항목**으로 있다. 별칭이 이걸 삼키면
    //    서로 다른 분류가 뭉쳐 되돌릴 수 없다.
    it("17. '직불'은 여전히 별개 분류다 — 별칭이 삼키지 않는다", () => {
        const p = preview([['홍길동', '직불', '논']])
        expect(p.items[0].rec.landClass1, "'직불'을 공익직불제로 뭉쳤다").toBe('직불')
    })

    it('18. 기존 값들은 그대로다', () => {
        for (const v of ['자체', '대표필지', '농가의뢰', '공익직불제', 'GAP']) {
            const p = preview([['홍길동', v, '논']])
            expect(p.items[0].rec.landClass1, `'${v}'가 바뀌었다`).toBe(v)
            expect(p.stats.err, `'${v}'가 오류가 됐다`).toBe(0)
        }
    })

    it('19. 여전히 알 수 없는 값은 오류로 남는다', () => {
        for (const v of ['직불(전략)', '직불일반', '일반', '공익']) {
            const p = preview([['홍길동', v, '논']])
            expect(p.stats.err, `'${v}'까지 받아들였다 — 별칭이 너무 넓다`).toBe(1)
        }
    })

    // 🚨 별칭 조회의 공백 제거를 **표준 목록 검사까지** 넓히면 '자 체'가 통과한다.
    //    그런데 저장되는 값은 원문 그대로라 '자 체'가 '자체'와 **다른 분류로 갈라진다** —
    //    별칭을 둔 이유가 바로 그걸 막는 것이었다.
    it('19-b. 표준값의 공백 변형은 통과시키지 않는다', () => {
        for (const v of ['자 체', '대표 필지', '공익 직불제']) {
            const p = preview([['홍길동', v, '논']])
            expect(p.stats.err, `'${v}'가 통과했다 — 원문 그대로 저장되어 분류가 갈라진다`).toBe(1)
        }
    })

    // 🚨 계획 리뷰(codex MAJOR)가 짚은 지점 — _recompute가 배치의 1차를 **행 값에서 직접**
    //    훑는다. 거기서 별칭을 안 맞추면 '직불(일반)'이라는 이름으로 커서를 받고
    //    저장은 '공익직불제'로 되어 **미리보기 번호와 저장 번호가 달라진다.**
    //    번호가 하나로 이어지는지로 확인한다.
    it('20. 별칭 행과 공익직불제 행이 같은 번호 시퀀스를 쓴다', () => {
        const p = preview([
            ['가', '직불(일반)', '논'],
            ['나', '공익직불제', '논'],
            ['다', '직불(일반)', '논'],
        ])
        expect(numbers(p), '별칭이 별개 시퀀스로 갈라졌다').toEqual(['1', '2', '3'])
    })

    it('20-b. 기존 공익직불제 로그의 다음 번호를 이어받는다', () => {
        const logs = [{ receptionNumber: '7', landClass1: '공익직불제', subCategory: '논' }]
        const p = preview([['가', '직불(일반)', '논']], { logs })
        expect(numbers(p), '별칭 행이 기존 공익직불제 번호를 못 봤다').toEqual(['8'])
    })

    // 🚨 같은 이유로 매핑 보정도 별칭을 알아야 한다. 모르면 그 열이
    //    "1차 값이 하나도 없다"로 보여 2차로 끌려간다.
    //
    // ⚠️ 별칭 값**만** 넣으면 이 테스트는 아무것도 구분하지 못한다 — 2차 값도 없어
    //    별칭을 몰라도 어차피 손대지 않기 때문이다(처음에 그렇게 썼다가 변이가 살아남았다).
    //    **별칭과 2차 값이 섞여야** 갈림길이 생긴다.
    it('21. 별칭과 2차 값이 섞여 있으면 1차 열을 빼앗기지 않는다', () => {
        const m = fns.refineMappingByValues(
            { name: 0, landClass1: 1 },
            [['가', '직불(일반)'], ['나', '논']]
        )
        expect(m.landClass1, '별칭을 못 알아보고 1차 열을 2차로 옮겼다').toBe(1)
        expect(m.subCategory, '2차가 그 열을 빼앗았다').toBeUndefined()
    })

    // 🚨 별칭을 목록에 **직접 넣는** 방식으로 고치면 창의 1차 선택지에 '직불(일반)'이
    //    나타난다. 사용자가 그걸 고르면 저장값이 '직불(일반)'이 되어
    //    통계·필터·채번에서 '공익직불제'와 갈라진다 — 별칭을 둔 이유가 사라진다.
    it('22. 별칭은 선택 가능한 1차 목록에 들어가지 않는다', () => {
        // 창의 1차 드롭다운이 이 목록으로 만들어진다. 여기 들어가면 사용자가 고를 수 있고,
        // 고른 값은 빈 칸 행의 저장값이 되므로 별칭이 그대로 저장된다.
        for (const alias of Object.keys(fns.LAND_CLASS1_ALIASES)) {
            expect(
                fns.LAND_CLASS1_OPTIONS,
                `'${alias}'을 고를 수 있게 됐다 — 저장값이 갈라진다`
            ).not.toContain(alias)
        }
    })

    it('22-b. 별칭이 가리키는 표준값은 목록에 있다', () => {
        for (const std of Object.values(fns.LAND_CLASS1_ALIASES)) {
            expect(fns.LAND_CLASS1_OPTIONS, `'${std}'가 목록에 없다 — 별칭이 갈 곳이 없다`)
                .toContain(std)
        }
    })
})
...
describe('값을 보고 1차↔2차를 바로잡는다', () => {
    ...
    // ⚠️ SLS-1-239에서 '직불(일반)'이 별칭으로 **알려진 값**이 됐다. 이 테스트가
    //    검증하는 것은 "정말 모르는 값"일 때의 동작이므로 표본을 바꿔 둔다.
    it('12. 알 수 없는 값뿐이면 손대지 않는다', () => {
        const m = refine({ name: 0, landClass1: 1 }, [['가', '직불(전략)']])
        expect(m.landClass1).toBe(1)
    })
