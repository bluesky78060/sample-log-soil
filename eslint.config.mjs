// ESLint flat config (v9) — 최소 도입 (SLS-1-96) + sourceType 정밀화 (SLS-1-103)
//   목적: 버그성 룰로 명백한 오류를 잡고, max-lines-per-function으로 거대 함수를 탐지.
//   무린팅 코드(29k LOC)라 스타일 룰은 끄고, 위험 룰만 error / 나머지는 warn.
//
// 환경 분리:
//   - 브라우저(렌더러·UMD): sourceType module + browser globals (대부분의 src)
//   - Electron 메인/preload: sourceType commonjs + node globals (index.js, preload.js)
//   - __dirname/Buffer는 node 파일 전용으로 격리 → 브라우저 파일에서 오용 시 no-undef로 탐지
//   - process/module/require는 dual-format(typeof 가드) 코드가 쓰므로 브라우저 블록에도 유지
import globals from 'globals';

// 브라우저·UMD 파일에 노출되는 프로젝트 전역(window.* 로 노출되는 shared 모듈 등)
const PROJECT_GLOBALS = {
    // dual-format(브라우저+CommonJS) 모듈의 export 가드용 + typeof process 가드
    module: 'readonly', require: 'readonly', process: 'readonly',
    // 선택적 전역(typeof 가드 후 호출 — 일부 환경/모듈에서만 노출)
    suggestRegionVillages: 'readonly', parseParcelAddress: 'readonly',
...
export default [
    ...
    // 브라우저·UMD 파일: ESM, browser globals (Electron 메인/preload 제외)
    {
        files: ['src/**/*.js'],
        ignores: NODE_FILES,
        languageOptions: {
            ...
            sourceType: 'module',
            globals: {
                ...globals.browser,  // crypto·structuredClone 등 포함 (__dirname/Buffer는 미포함)
                ...PROJECT_GLOBALS,
            },
        },
    },
    // Electron 메인/preload: CommonJS, node globals(__dirname/Buffer/process/module/require 등)
    {
        files: NODE_FILES,
        languageOptions: {
            ecmaVersion: 2022,
