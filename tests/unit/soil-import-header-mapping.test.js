...
describe('막는 것 — 조용히 틀린 값이 들어가던 헤더', () => {
    ...
    it('2. 표기 변형도 함께 막힌다', () => {
        for (const h of ['필지 구분', '필지-구분', ' 필지구분 ', '필지(구분)', '필지유형', '본필지구분', '경지구분1차']) {
            expect(mapOne(h), `'${h}'가 새어 나갔다`).toBeNull()
        }
    })

    // 🚨 이 앱에서 경지구분 = 1차(자체/대표필지/농가의뢰/공익직불제).
    //    2차(논/밭/과수/시설)는 '구분'이라고 부른다. 1차는 가져오기 대상 필드가 아니다.
    it('3. 경지구분(1차)은 구분에 붙지 않는다', () => {
        for (const h of ['경지구분', '경지구분1차', '경지구분 1차', '경지구분1']) {
            expect(mapOne(h), `'${h}'가 구분(2차)에 붙었다`).toBeNull()
        }
    })
})
...
describe('막지 않는 것 — 과잉 차단 방지', () => {
    ...
    it('5. 경지구분2차는 정확히 우리 subCategory다', () => {
        for (const h of ['경지구분2차', '경지구분 2차', '경지구분2']) {
            expect(mapOne(h), `'${h}'까지 막았다`).toBe('subCategory')
        }
    })
