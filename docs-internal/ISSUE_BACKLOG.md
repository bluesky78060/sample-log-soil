...
### F-2. 공익직불제 기준년도 (`GONGIK_BASE_YEAR_OPTIONS`) — ✅ 현행 유지 결정 (2026-06-05)
- **파일**: `src/soil/soil-script.js:35` `['2024…','2025…','2026…','2027…' 토양화학성분 기준]`
- **정정**: "농진청 정식 이행점검명 목록 대기"는 **오해**였음. 기준년도는 고정 명칭 목록이 아니라 **이행점검 기준이 바뀐 해의 연도**가 그대로 기준값이 되는 구조(작년까지 2022 → 2025년 기준 변경으로 현재 2025). 새 기준이 생기면 그 해 연도가 추가됨.
- **결정(사용자, 2026-06-05)**: 현재 2024~2027 연도 범위를 **그대로 유지**(미래 연도까지 커버). 별도 교체/대기 불필요 → 과제 종결.
