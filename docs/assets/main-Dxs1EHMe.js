...
설정 페이지에서 인증 파일을 등록해주세요.`);return}I.classList.add("syncing");const e=new Date().getFullYear(),t=[],o=C.length*(e-A+1);let n=0;H(C);try{for(let r=0;r<C.length;r++){const i=C[r];let c=0,M=[];x(r,"syncing","동기화 중...");for(let d=A;d<=e;d++){try{let l,p=!1;if(typeof window.firestoreDb.getAllWithMeta=="function"){const m=await window.firestoreDb.getAllWithMeta(i.type,d);l=m.documents,p=m.fromCache===!0}else l=await window.firestoreDb.getAll(i.type,d);const L=`${i.storagePrefix}_${d}`;if(l&&l.length>0){const m=(h=window.SyncUtils)!=null&&h.smartMerge?(()=>{let s;try{s=JSON.parse(localStorage.getItem(L)||"[]")}catch{s=[]}return Array.isArray(s)||(s=[]),(window.SyncUtils.mergeCloudData?window.SyncUtils.mergeCloudData(s,l,{fromCache:p}):{data:window.SyncUtils.smartMerge(s,l,{allowDeletions:!p}).data}).data})():l;localStorage.setItem(L,JSON.stringify(m)),c+=m.length,M.push({year:d,count:m.length})}}catch(l){(((S=window.logger)==null?void 0:S.error)||console.error)(`${i.name} ${d}년 동기화 오류:`,l)}n++,R(n,o,`${i.name} ${d}년...`)}const $=c>0?`${c}건 완료`:"데이터 없음";x(r,c>0?"success":"",$),t.push({type:i.type,name:i.name,icon:i.icon,totalCount:c,yearsWithData:M,success:!0})}Y(t)}catch(r){(((E=window.logger)==null?void 0:E.error)||console.error)("동기화 오류:",r),alert("동기화 중 오류가 발생했습니다: "+r.message)}finally{I.classList.remove("syncing")}}function H(e){let t=`
    ...
    `;e.forEach((o,n)=>{t+=`
        ...
        `}),N.innerHTML=t,u.classList.add("show")}function R(e,t,o){const n=Math.round(e/t*100),a=document.getElementById("syncProgressFill"),g=document.getElementById("syncProgressLabel"),y=document.getElementById("syncProgressPercent");a&&(a.style.width=`${n}%`),g&&(g.textContent=o),y&&(y.textContent=`${n}%`)}function x(e,t,o){const n=document.getElementById(`syncItem${e}`),a=document.getElementById(`syncStatus${e}`);n&&(n.className=`sync-result-item ${t}`),a&&(a.className=`sync-result-count ${t}`,a.textContent=o)}function Y(e){let t=0,o="";e.forEach(n=>{t+=n.totalCount;const a=n.totalCount>0?"success":"";o+=`
    ...
    `,N.innerHTML=o,u.classList.add("show")}I==null||I.addEventListener("click",F);document.addEventListener("DOMContentLoaded",()=>{var e;if((e=window.electronAPI)!=null&&e.isElectron){const t=document.getElementById("feedbackNavBtn");t&&(t.style.display="")}});
